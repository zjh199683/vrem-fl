...
class FedNovaOptimizer(Optimizer):
    r"""Implements federated normalized averaging (FedNova). See
    `Tackling the Objective Inconsistency Problem in Heterogeneous Federated Optimization`__.

    This class is addapted from https://github.com/JYWa/FedNova/blob/master/distoptim/FedNova.py
    ...
    """
    def __init__(self, params, lr, mu=0., momentum=0., dampening=0.,
                 weight_decay=0., nesterov=False):
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                if 'cum_grad' not in param_state:
                    ...
                    # I comment the next line, because we don't really need to multiply by `lr`,
                    # since it is included in the optimizer of the global model

                    # param_state['cum_grad'].mul_(lr)

                else:
                    param_state['cum_grad'].add_(torch.clone(d_p))
    ...
    def swap_cum_grad(self):
        """
        sets the gradients of each parameter to have the cumulative gradients normalized by the `local_normalizing_vec`
        """
        for group in self.param_groups:
            for p in group['params']:
                param_state = self.state[p]

                p.grad = param_state['cum_grad'].data.clone() / self.local_normalizing_vec
...
def get_optimizer(optimizer_name, model, lr_initial, mu=0., weight_decay=0., name=None):
    """
    ...
    if optimizer_name == "adam":
        ...
        if name == 'apolloscape':
            return optim.SGD(
                [{'params': model.mobilenetv3_deeplabv3.backbone.parameters(), 'lr': lr_initial},
                 {'params': model.mobilenetv3_deeplabv3.classifier.parameters(), 'lr': 10 * lr_initial}],
                momentum=0.,
                weight_decay=weight_decay)
        return optim.SGD(
            [param for param in model.parameters() if param.requires_grad],
...
def get_lr_scheduler(optimizer, scheduler_name, n_rounds=None):
    ...
    if scheduler_name == "sqrt":
        ...
        return optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=10000, eta_min=0)
