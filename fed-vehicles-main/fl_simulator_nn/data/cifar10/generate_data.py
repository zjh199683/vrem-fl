...
def save_meta_data(clients_indices, path_):
    num_samples = []
    for client_indices in clients_indices:
        num_samples.append(len(client_indices))

    metadata = \
        {
            "clients_weights": np.array(num_samples) / sum(num_samples)
        }
...
def main():
    ...
    save_meta_data(train_clients_indices, os.path.join(PATH, "meta.pkl"))
